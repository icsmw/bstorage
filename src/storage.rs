...
impl BinStorage {
    ...
    pub fn unpack<P: AsRef<Path>>(bundle: P) -> Result<Self, E> {
        ...
        let parent = bundle.parent().ok_or(E::NoParentOfStorageFile)?;
        let storage_dir = parent.join(Uuid::new_v4().to_string());
        ...
        let map_size = u64::from_le_bytes(buffer) as usize;
        let mut buffer: Vec<u8> = Vec::with_capacity(map_size);
        file.seek(SeekFrom::Start(U64_SIZE as u64))?;
        file.read_exact(&mut buffer)?;
        ...
        for (key, (filename, from, to)) in location {
            ...
            let mut content: Vec<u8> = Vec::with_capacity((to - from) as usize);
            file.seek(SeekFrom::Start(from))?;
            file.read_exact(&mut content)?;
            let mut record = OpenOptions::new()
                .write(true)
                .truncate(true)
                .open(storage_dir.join(&filename))?;
        ...
        let mut map_file = OpenOptions::new()
            .write(true)
            .truncate(true)
            .open(storage_dir.join(MAP_FILE_NAME))?;
        let buffer = bincode::serialize(&map)?;
        map_file.write_all(&buffer)?;
        map_file.flush()?;
        let mut storage = Self::open(storage_dir)?;
        storage.bundle = Some(bundle);
        Ok(storage)
    }

    fn pack<P: AsRef<Path>>(&mut self, bundle_file: P) -> Result<(), E> {
